...
class 'LastStand_BuildSentry' (Weapon)

LastStand_BuildSentry.kMapName = "buildsentry"
...
function LastStand_BuildSentry:OnCreate()
...
function LastStand_BuildSentry:OnInitialized()
...
function LastStand_BuildSentry:GetIsValidRecipient(recipient)

    if self:GetParent() == nil and recipient and not GetIsVortexed(recipient) and recipient:isa("Marine") then
    
        local buildSentry = recipient:GetWeapon(LastStand_BuildSentry.kMapName)
...
function LastStand_BuildSentry:GetDropStructureId()
    return kTechId.Sentry
end

function LastStand_BuildSentry:GetViewModelName()
    return nil
end

function LastStand_BuildSentry:GetAnimationGraphName()
    return kAnimationGraph
end

function LastStand_BuildSentry:GetSuffixName()
    return "sentry"
end

function LastStand_BuildSentry:GetDropClassName()
    return "Sentry"
end

function LastStand_BuildSentry:GetDropMapName()
    return Sentry.kMapName
end

function LastStand_BuildSentry:GetHUDSlot()
    return 4
end

function LastStand_BuildSentry:Build()
...
function LastStand_BuildSentry:OnTag(tagName)
...
function LastStand_BuildSentry:OnPrimaryAttackEnd()
    self.droppingSentry = false
end

function LastStand_BuildSentry:GetIsDroppable()
    return true
end

function LastStand_BuildSentry:OnPrimaryAttack(player)
...
function LastStand_BuildSentry:PerformPrimaryAttack(player)
...
function LastStand_BuildSentry:OnHolster(player, previousWeaponMapName)
...
function LastStand_BuildSentry:OnDraw(player, previousWeaponMapName)
...
function LastStand_BuildSentry:Dropped(prevOwner)
...
function LastStand_BuildSentry:GetPositionForStructure(player)
...
function LastStand_BuildSentry:GetGhostModelName()
    return LookupTechData(self:GetDropStructureId(), kTechDataModel)
end

function LastStand_BuildSentry:OnUpdateAnimationInput(modelMixin)
...
if Client then
    ...
    function LastStand_BuildSentry:OnProcessIntermediate()
    ...
    function LastStand_BuildSentry:GetUIDisplaySettings()
        return { xSize = 256, ySize = 417, script = "lua/GUIMineDisplay.lua" }
    end
    
end

function LastStand_BuildSentry:GetShowGhostModel()
    return self.showGhost
end

function LastStand_BuildSentry:GetGhostModelCoords()
    return self.ghostCoords
end   

function LastStand_BuildSentry:GetIsPlacementValid()
    return self.placementValid
end

Shared.LinkClassToMap("LastStand_BuildSentry", LastStand_BuildSentry.kMapName, networkVars)
